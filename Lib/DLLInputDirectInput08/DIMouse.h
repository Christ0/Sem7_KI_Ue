...
public:
	...
	bool ButtonPressed(int iButton); // true, falls Taste iButton (Tastennummer gemäß DirectX) gedrückt wurde, ansonsten false
	bool ButtonDown(int iButton); // true, falls Taste iButton (Tastennummer gemäß DirectX) gedrückt wurde, ansonsten false
	bool ButtonUp(int iButton); // true, falls Taste iButton (Tastennummer gemäß DirectX) gedrückt wurde, ansonsten false

	void ReSize(int iWidthWindow, int iHeightWindow); // Sollte immer aufgerifen werden, wenn Bildschirmgröße sich verändert hat (geschieht automatisch, wenn an Frame angehängt)
